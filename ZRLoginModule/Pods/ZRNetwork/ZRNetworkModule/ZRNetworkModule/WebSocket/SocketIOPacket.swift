...
internal class SocketIOPacket: NSObject {
    ...
    internal var jsonData: Data {
        ...
        /// req_id
        header["reqId"] = String(self.reqId)
        /// path
    ...
    /// 初始化默认超时时间 60s
    convenience init(_ reqId: Int, path: String, param: [String : Any]?, timeout: Int = 60, closure: WebSocketClosure? = nil) {
        self.init()
        self.reqId = String(reqId)
        self.path = path
        self.param = param
